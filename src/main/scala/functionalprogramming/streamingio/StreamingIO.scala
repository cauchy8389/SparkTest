package functionalprogramming.streamingio

import functionalprogramming.iomonad.{IO,Monad,Free,unsafePerformIO}
...
object SimpleStreamTransducers {
  ...
  object Process {
    ...
    import functionalprogramming.iomonad.Monad
...
object GeneralizedStreamTransducers {
  ...
  object Process {
    ...
    def runLog[O](src: Process[IO,O]): IO[IndexedSeq[O]] = IO {
      ...
      def go(cur: Process[IO,O], acc: IndexedSeq[O]): IndexedSeq[O] =
        cur match {
          ...
          case Await(req,recv) =>
            val next =
              try recv(Right(functionalprogramming.iomonad.unsafePerformIO(req)(E)))
    ...
    import functionalprogramming.iomonad.IO0.fahrenheitToCelsius

    val converter: Process[IO,Unit] =
      lines("D:/WorkData/scaladata/fahrenheit.txt").
      filter(line =>{
        println(line)
        !line.startsWith("#") && !line.trim.isEmpty
      }).
      map(line => {
        println(line.toDouble)
        fahrenheitToCelsius(line.toDouble).toString
      }).
      pipe(intersperse("\n")).
      to(fileW("D:/WorkData/scaladata/celsius.txt")).
      drain
    ...
    val convertAll: Process[IO,Unit] = (for {
      out <- fileW("D:/WorkData/scaladata/celsius.txt").once
      file <- lines("D:/WorkData/scaladata/fahrenheits.txt")
      _ <- lines(file).
