...
object TFIDFExtraction {
  ...
  def main(args: Array[String]) {
    ...
    val path = "file:///F:/CodingResource/BigData/spark-ml-branch-ed2/Chapter_10/scala-2.0.x/data/20news-bydate-train/alt.atheism/*"
    ...
    */
    ...
    val hockeyText = rdd.filter { case (file, text) => text.contains("racism") }
    ...
    val hockey1 = hockeyTfIdf.sample(true, 1.0, 42).first.asInstanceOf[SV]
    ...
    val graphicsText = rdd.filter { case (file, text) => text.contains("issue") }
    val graphicsTF = graphicsText.mapValues(doc => hashingTF.transform(tokenize(doc)))
    val graphicsTfIdf = idf.transform(graphicsTF.map(_._2))
    val graphics = graphicsTfIdf.sample(true, 1.0, 42).first.asInstanceOf[SV]
    ...
    val baseballText = rdd.filter { case (file, text) => text.contains("food") }
    val baseballTF = baseballText.mapValues(doc => hashingTF.transform(tokenize(doc)))
    val baseballTfIdf = idf.transform(baseballTF.map(_._2))
    val baseball = baseballTfIdf.sample(true, 1.0, 42).first.asInstanceOf[SV]
